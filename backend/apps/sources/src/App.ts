...
class App {
    static async run() {
        ...
        await fastifyServer.register(multipart, { limits: { fileSize: 20 * 1024 * 1024 }, attachFieldsToBody: false });
        ...
        for await (const router of [...routeAdmins, ...routeCustomers, ...routesSites]) {
            ...
            fastifyServer.route({
                ...
                handler: async (req: FastifyRequest, reply: FastifyReply) => {
                    try {
                        ...
                        const classController = new router.controller(req, reply, fastifyServer, mysqlPool);
                        if (classController[router.action] === undefined) {
                            throw new Error(`Cannot method ${router.action}`);
                        }
                        const code: number = (req.params as { code: number }).code;
                        // const mysqlPool = (fastifyServer as any).mysql;

                        classController.request = new Request(req, reply, mysqlPool);
                        ...
                        return reply.code(400).send({ msg: error.message, status: 400 });
                    }
                }
            });
        }
        ...
        const start = async () => {
            try {
                await fastifyServer.listen({ port: Number(process.env.PORT || 3000), host: '0.0.0.0' });
