...
import { MenuItemT } from '@/types/common.types';
import { apiFetchSites } from '@/lib/api/api';
import Link from 'next/link';

function Footer() {
  const { config } = useConfig();
  const [menuData, setMenuData] = React.useState<MenuItemT[]>([]);
  const handleFetchMenuData = async () => {
    try {
      const data = await apiFetchSites('/menu/getFuncMenu?menu_type=2')
      setMenuData(data);
    } catch (error) {
  ...
  const getLinkHref = (item: MenuItemT) => {
              ...
              <div className="row g-md-4 g-3">
                ...
                <div className="col-xl-2 col-lg-4 col-md-4 col-sm-6">
                  ...
                  <div className="footer-contain">
                    <ul>
                      {
                        menuData?.[0]?.subs?.map((item, index) => (
                          <li key={index}>
                            <Link href={getLinkHref(item) || "#"} className="text-content">{item.name}</Link>
                          </li>
                        ))
                ...
                <div className="col-xl col-lg-4 col-sm-6">
                  ...
                  <div className="footer-contain">
                    <ul>
                      ...
                      {
                        menuData?.[1]?.subs?.map((item, index) => (
                          <li key={index}>
                            <Link href={getLinkHref(item) || "#"} className="text-content">{item.name}</Link>
                          </li>
